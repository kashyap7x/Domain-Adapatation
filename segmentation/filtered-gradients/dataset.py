...
def make_CityScapes_coarse(mode, root):
...
class CityScapes_coarse(torchdata.Dataset):
    def __init__(self, mode, root, cropSize=512, ignore_label=-1, max_sample=-1, is_train=1):
        self.list_sample = make_CityScapes_coarse(mode, root)
        self.mode = mode
        self.cropSize = cropSize
        self.is_train = is_train
        self.img_transform = transforms.Compose([
...
class CityScapes_both(torchdata.Dataset):
    def __init__(self, mode, root, cropSize=512, ignore_label=-1, max_sample=-1, is_train=1):
        self.list_sample = make_CityScapes_coarse('train_extra', root) + make_CityScapes('train', root)
        self.mode = mode
        self.cropSize = cropSize
        self.is_train = is_train
        self.img_transform = transforms.Compose([
...
class BDD(torchdata.Dataset):
    ...
    def selfcheck(self, list_sample):
        ...
        print('{} samples mismatch in input and label, they are ignored'.format(counter))
